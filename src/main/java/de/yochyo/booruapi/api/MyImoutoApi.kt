...
class MyImoutoApi(url: String) : DanbooruApi(url) {
    private val utils = MoebooruUtils()


    override suspend fun getMatchingTags(beginSequence: String, limit: Int): List<Tag>? {
        println("${url}tag.json?name=$beginSequence*&limit=$limit&search[order]=count")
    ...
    override fun getTagFromJson(json: JSONObject): Tag? {
        return try {
            Tag(this, json.getString("name"), json.getInt("tag_type"), json.getInt("post_count"))
        } catch (e: Exception) {
            null
        }
    }
            ...
            for (l in splitLines) {
                try {
                    ...
                    if (type != Tag.UNKNOWN) {
                        ...
                        tags += Tag(this@MyImoutoApi, name, type, 0)
                    }
                } catch (e: Exception) {
                }
            }
